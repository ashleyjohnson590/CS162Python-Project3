...
class test_items(unittest.TestCase):
    ...
    def testLibrary(self):
        ...
        AlbumArtist = "Beatles"
        myAlbum = Album(AlbumID, AlbumTitle, AlbumArtist)
        BookID = 1
        BookTitle = "war and peace"
        BookAuthor = "Tolstoy"
        myBook = Book(BookID, BookTitle, BookAuthor)
        MovieID = 3
        MovieTitle = "star wars"
        MovieDirector = "bruce lee"
        myMovie = Movie(MovieID, MovieTitle, MovieDirector)
        patron1ID = 123
        patron1name = "Ashley"
        ...
        myItems = patron2.get_checked_out_items()
        myLibrary = Library(libraryPatrons, libraryItems)
        status=myLibrary.check_out_library_item(patron2ID,BookID)
        myItems = patron2.get_checked_out_items()
        self.assertEqual(status, "check out successful")
        status=myLibrary.check_out_library_item(patron2ID,MovieID)
        returnStatus = myLibrary.return_library_item(BookID)


        for i in range(10):
            myLibrary.increment_current_date()
        status = myLibrary.check_out_library_item(patron2ID, AlbumID)
        for i in range(15):
            myLibrary.increment_current_date()
        p2_fine = patron2.print_fine_amount()
        self.assertEqual("$1.90", p2_fine)
        pay_fine = myLibrary.pay_fine(patron2ID, 1.00)
        self.assertEqual(pay_fine, "payment successful")
        p2_fine = patron2.print_fine_amount()
        print(p2_fine)
        self.assertEqual("$0.90", p2_fine)
